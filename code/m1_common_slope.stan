...
parameters {
  ...
  real beta; // common slope across blocks
  ...
  // misclassification (feature-level; pooled by block) on logit scale
...
transformed parameters {
  vector[N_features] gplus;   // false positive prob per feature
  vector[N_features] gminus;  // false negative prob per feature
...
model {
  ...
  // likelihood with misclassification
  for (i in 1:N_items) {
    real eta_i = inv_logit(z[i]);
    for (j in 1:N_features) {
      int b = block_id[j];
      real lp = alpha_b[b] + beta * eta_i + u_feat[j] + v_item[i];
      real p  = inv_logit(lp);
...
generated quantities {
  ...
  for (i in 1:N_items) {
    for (j in 1:N_features) {
      int b = block_id[j];
      real lp = alpha_b[b] + beta * eta[i] + u_feat[j] + v_item[i];
      real p  = inv_logit(lp);
