...
This measurement-level stability, rather than decisive categorization, is compatible with viewing grammatical categories as homeostatic property clusters whose bundles can diverge at the edge. The methodology provides a template for transparent investigation of small-$n$ categorization problems, showing how boundary phenomena can be studied rigorously without forcing artificial binary decisions. Within an \textit{CGEL}-informed measurement model, reciprocals occupy the interface between pronouns and compound determinatives rather than motivating a decisive recategorization.
\fi
\end{abstract}
\ifcjlsubmission
